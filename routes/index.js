...
router.post("/good/:id/bid", isLoggedIn, async (req, res, next) => {
  try {
    ...
    if (good.price >= bid) {
      return res.status(403).send("you need up price than original");
    }
    if (new Date(good.createdAt).valueOf() + 24 * 60 * 60 * 1000 < new Date()) {
      return res.status(403).send("finished aution ㅜㅜ");
    }
    if (good.Auctions[0] && good.Auctions[0].bid >= bid) {
      return res.status(403).send("you need up price than before");
    }
